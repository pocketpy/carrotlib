...
def linear(a: float, b: float, k: float):
    return a + k * (b - a)

def _unsigned_shift_right(x: int, y: int):
    """The `>>>` operator"""
    if x >= 0:
        return x >> y
...
class Perlin:
    ...
    def __init__(self):
        self.repeat = 0x7fffffff
        self.gradients = []
        for i in range(NGRADS):
            self.gradients.append(GRADIENTS[i * 3] * 2.12)
            self.gradients.append(GRADIENTS[i * 3 + 1] * 2.12)
            self.gradients.append(GRADIENTS[i * 3 + 2] * 2.12)
            self.gradients.append(0)    # padding

    def gradient3DAt(self, x: float, y: float, z: float, ix: int, iy: int, iz: int, seed: int):
        index = seed * 1013 + (ix % self.repeat) * 1619 + (iy % self.repeat) * 31337 + iz * 6971
        index = ((index ^ _unsigned_shift_right(index, 8)) & 0xFF)
        index <<= 2
        gx = self.gradients[index]
        gy = self.gradients[index + 1]
        gz = self.gradients[index + 2]
        return gx * (x - ix) + gy * (y - iy) + gz * (z - iz)

    def gradient1DAt(self, x: float, ix: int, seed: int):
        index = seed * 1013 + (ix % self.repeat) * 1619
        index = ((index ^ _unsigned_shift_right(index, 8)) & 0xFF)
        gx = self.gradients[index << 2]
        return gx * (x - ix)

    def gradientAt(self, x: float, y: float, ix: int, iy: int, seed: int):
        index = seed * 1013 + (ix % self.repeat) * 1619 + (iy % self.repeat) * 31337
        index = ((index ^ _unsigned_shift_right(index, 8)) & 0xFF)
        gx = self.gradients[index << 2]
        gy = self.gradients[(index << 2) + 1]
        return gx * (x - ix) + gy * (y - iy)

    def adjustScale(self, size: int, scale: float):
        self.repeat = int(size * scale)
        return self.repeat / size

    def gradient3D(self, seed: int, x: float, y: float, z: float):
        ix, xs = int(x), scurve(x - ix)
        iy, ys = int(y), scurve(y - iy)
        iz, zs = int(z), scurve(z - iz)

        ga = self.gradient3DAt(x, y, z, ix, iy, iz, seed)
        gb = self.gradient3DAt(x, y, z, ix + 1, iy, iz, seed)
        gc = self.gradient3DAt(x, y, z, ix, iy + 1, iz, seed)
        gd = self.gradient3DAt(x, y, z, ix + 1, iy + 1, iz, seed)
        v1 = linear(linear(ga, gb, xs), linear(gc, gd, xs), ys)

        ga = self.gradient3DAt(x, y, z, ix, iy, iz + 1, seed)
        gb = self.gradient3DAt(x, y, z, ix + 1, iy, iz + 1, seed)
        gc = self.gradient3DAt(x, y, z, ix, iy + 1, iz + 1, seed)
        gd = self.gradient3DAt(x, y, z, ix + 1, iy + 1, iz + 1, seed)
        v2 = linear(linear(ga, gb, xs), linear(gc, gd, xs), ys)

        return linear(v1, v2, zs)

    def gradient1D(self, seed: int, x: float):
        ix, xs = int(x), scurve(x - ix)
        ga = self.gradient1DAt(x, ix, seed)
        gb = self.gradient1DAt(x, ix + 1, seed)
        return linear(ga, gb, xs)
    
    def gradient(self, seed: int, x: float, y: float):
        return self.inlineGradient(seed, x, y)

    def inlineGradient(self, seed: int, x: float, y: float):
        ix = int(x); xs = scurve(x - ix)
        iy = int(y); ys = scurve(y - iy)
        ga = self.gradientAt(x, y, ix, iy, seed)
        gb = self.gradientAt(x, y, ix + 1, iy, seed)
        gc = self.gradientAt(x, y, ix, iy + 1, seed)
        gd = self.gradientAt(x, y, ix + 1, iy + 1, seed)
        return linear(linear(ga, gb, xs), linear(gc, gd, xs), ys)

    def perlin(self, seed: int, x: float, y: float, octaves: int, persist: float = 0.5, lacunarity: float = 2.0):
        v = 0.0
        k = 1.0
        sum = 0.0
        for i in range(octaves):
            v += self.inlineGradient(seed + i, x, y) * k
            sum += k
            k *= persist
            x *= lacunarity
            y *= lacunarity
    ...
    def perlin1D(self, seed: int, x: float, octaves: int, persist: float = 0.5, lacunarity: float = 2.0):
        v = 0.0
        k = 1.0
        sum = 0.0
        for i in range(octaves):
            v += self.gradient1D(seed + i, x) * k
            sum += k
            k *= persist
            x *= lacunarity
        if self.normalize:
    ...
    def ridged(self, seed: int, x: float, y: float, octaves: int, offset: float = 0.5, gain: float = 2.0, persist: float = 0.5, lacunarity: float = 2.0):
        v = 0.0
        p = 1.0
        s = lacunarity
        weight = 1.0
        tot = 0.0
        for i in range(octaves):
            g = self.inlineGradient(seed + i, x * s, y * s) * p
            g = offset - abs(g)
            g *= g
            g *= weight
            v += g * s
    ...
    def thresholdValue(self, p: float):
    ...
    def maxValue(self, octaves: int, persist: float):
        tot = 0.0
        n = 1.0
        for i in range(octaves):
            tot += n
